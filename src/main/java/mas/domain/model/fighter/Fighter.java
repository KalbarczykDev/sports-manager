...
public class Fighter extends Person {
    ...
    public Salary getSalary() { //atr pochodny
        ...
        double salary = (basicSalary.getValue() * (titles.size() + 1)) + (basicSalary.getValue() * (LocalDateTime.now().getYear() - dateOfJoining.getValue().getYear()));
    ...
    public String toString() { //przeciążenie
        return "Fighter{" + ", name='" + getName() + '\'' + ", surname='" + getSurname() + '\'' + ", address=" + getAddress() + ", titles=" + titles + ", contracts=" + contracts.stream().map(Contract::getExpiresAt).toList() + ", coaches=" + coaches.entrySet().stream().map(entry -> entry.getKey() + ": " + entry.getValue().getName() + " " + entry.getValue().getSurname()).toList() + ", sponsors=" + sponsors.stream().map(Sponsor::getCompanyName).toList() + ", dateOfJoining=" + dateOfJoining + '}';
    }
